...
Bounce button14 = Bounce(14, 5);  //verifica
Bounce button15 = Bounce(15, 5);  //verifica
...
void setup() {
  ...
  pinMode(13, INPUT_PULLUP); // Teensy 3.6 and 4.0 LED ???, may need 1k resistor pullup
  pinMode(14, INPUT_PULLUP); //verifica
  pinMode(15, INPUT_PULLUP); //verifica
}
...
void loop() {
  ...
  if (button0.fallingEdge()) {
    usbMIDI.sendNoteOn(59, 99, channel);  // 59 = C4 ??? se si sistema i commenti degli altri
  }
  if (button1.fallingEdge()) {
    usbMIDI.sendNoteOn(60, 99, channel);  // 61 = C#4
  }
  ...
  if (button12.fallingEdge()) {
    usbMIDI.sendNoteOn(71, 99, channel);  // 72 = C5
  }
  if (button13.fallingEdge()) {
    usbMIDI.sendNoteOn(72, 99, channel);  // 73 = C#5    ricontrolla
  }
  if (button14.fallingEdge()) {
    usbMIDI.sendNoteOn(73, 99, channel);  // 74 = D5   ricontrolla
  }
  if (button15.fallingEdge()) {
    usbMIDI.sendNoteOn(74, 99, channel);  // 75 = D#5    ricontrolla
  }
  ...
  if (button0.risingEdge()) {
    usbMIDI.sendNoteOff(59, 0, channel);  // 60 = C4
  }
  ...
  if (button12.fallingEdge()) {
    usbMIDI.sendNoteOff(72, 0, channel);  // 72 = C5
  }
  if (button13.fallingEdge()) {
    usbMIDI.sendNoteOff(73, 0, channel);  // 73 = C#5
  }
  if (button14.fallingEdge()) {
    usbMIDI.sendNoteOff(74, 0, channel);  // 74 = D5
  }
  if (button15.fallingEdge()) {
    usbMIDI.sendNoteOff(75, 0, channel);  // 75 = D#5
  }
